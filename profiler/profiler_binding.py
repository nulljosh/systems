...
class CPUProfiler:
 ...
 if result == 0:
 print(f"[x] Flame graph JSON saved to {output_path}")
 return True
 else:
 ...
 if result == 0:
 print(f"[x] Summary saved to {output_path}")
 return True
 else:
...
class FlameGraphConverter:
 ...
 print(f"[x] Flame graph text format saved to {output_path}")
