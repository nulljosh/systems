...
static void parser_error(Parser *p, const char *msg) {
    ...
    if (t) {
        ...
        fprintf(stderr, "ParseError: unexpected end of file: %s\n", msg);
    }
    exit(1);
}
...
static ASTNode *parse_statement(Parser *p) {
    ...
    if (match(p, TOKEN_LET)) {
        ...
        if (match(p, TOKEN_LBRACKET)) {
            ...
            while (!match(p, TOKEN_RBRACKET) && !match(p, TOKEN_EOF)) {
                if (match(p, TOKEN_DOT) &&
                    peek(p, 1) && peek(p, 1)->type == TOKEN_DOT &&
                    peek(p, 2) && peek(p, 2)->type == TOKEN_DOT) {
                    advance_tok(p);
                    advance_tok(p);
                    advance_tok(p);
                    Token *rest = consume(p, TOKEN_IDENTIFIER, "Expected rest variable name");
                    rest_name = strdup(rest->value);
                    break;
                }

                Token *name = consume(p, TOKEN_IDENTIFIER, "Expected variable name in array destructuring");
                if (cnt >= cap) {
                    cap *= 2;
                    names = realloc(names, sizeof(char *) * (size_t)cap);
                }
                names[cnt++] = strdup(name->value);
            ...
            consume(p, TOKEN_RBRACKET, "Expected ']' after array destructuring");
            consume(p, TOKEN_ASSIGN, "Expected '=' in assignment");
            ASTNode *val = parse_expression(p);
            optional_semicolon(p);

            ASTNode *n = alloc_node(NODE_DESTRUCTURE_ARRAY, line, col);
            n->as.destructure_arr.names = names;
            n->as.destructure_arr.name_count = cnt;
            n->as.destructure_arr.rest_name = rest_name;
            n->as.destructure_arr.value = val;
            return n;
        }
        ...
        if (match(p, TOKEN_LBRACE)) {
            ...
            while (!match(p, TOKEN_RBRACE) && !match(p, TOKEN_EOF)) {
                Token *name = consume(p, TOKEN_IDENTIFIER, "Expected variable name in object destructuring");
                if (cnt >= cap) {
                    cap *= 2;
                    names = realloc(names, sizeof(char *) * (size_t)cap);
                }
            ...
            consume(p, TOKEN_RBRACE, "Expected '}' after object destructuring");
            consume(p, TOKEN_ASSIGN, "Expected '=' in assignment");
            ASTNode *val = parse_expression(p);
            optional_semicolon(p);

            ASTNode *n = alloc_node(NODE_DESTRUCTURE_OBJECT, line, col);
            n->as.destructure_obj.names = names;
            n->as.destructure_obj.name_count = cnt;
            n->as.destructure_obj.value = val;
            return n;
        }
    ...
    switch (node->type) {
        ...
        case NODE_DESTRUCTURE_ARRAY:
            for (int i = 0; i < node->as.destructure_arr.name_count; i++)
                free(node->as.destructure_arr.names[i]);
            free(node->as.destructure_arr.names);
            free(node->as.destructure_arr.rest_name);
            ast_free(node->as.destructure_arr.value);
            break;

        case NODE_DESTRUCTURE_OBJECT:
            for (int i = 0; i < node->as.destructure_obj.name_count; i++)
                free(node->as.destructure_obj.names[i]);
            free(node->as.destructure_obj.names);
            ast_free(node->as.destructure_obj.value);
            break;
    }
    free(node);
}
