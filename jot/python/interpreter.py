    ...
    def evaluate(self, node: ASTNode) -> Any:
        ...
        if isinstance(node, Number):
            ...
            if not isinstance(value, list):
                self.error('Array destructuring requires array')
            for i, name in enumerate(node.names):
                if i < len(value):
                    self.environment.define(name, value[i])
                else:
                    self.environment.define(name, None)
            if node.rest_name:
                rest = value[len(node.names):] if len(value) > len(node.names) else []
                self.environment.define(node.rest_name, rest)
            return None

        elif isinstance(node, ObjectDestructure):
            value = self.evaluate(node.value)
            if not isinstance(value, dict):
                self.error('Object destructuring requires object')
            for name in node.names:
                if name in value:
                    self.environment.define(name, value[name])
                else:
                    self.environment.define(name, None)
            return None
