...
def profile_function(output_dir: Optional[str] = None) -> Callable:
 ...
 print(f"\n[x] Profile saved to {config.output_dir}")
 return result
...
""")
 ...
 print(f"\n[x] Profiles saved to {config.output_dir}")
 ...
 profiler.save_json(json_file)
 profiler.save_summary(txt_file)
 print(f"[x] Saved to {config.output_dir}")
 elif cmd == "help":
 print("""
Commands:
