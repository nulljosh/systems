...
@dataclass
class ArrayDestructure(ASTNode):
    names: List[str]
    rest_name: Optional[str]
    value: ASTNode

@dataclass
class ObjectDestructure(ASTNode):
    names: List[str]
    value: ASTNode
...
class Parser:
    ...
    def error(self, message: str):
        if self.position < len(self.tokens):
            ...
            raise SyntaxError(f"ParseError line {token.line}:{token.column}: {message}")
        else:
            raise SyntaxError(f"ParseError: unexpected end of file: {message}")
    ...
    def parse_statement(self) -> Optional[ASTNode]:
        ...
        if self.match(TokenType.LET):
                ...
                if not self.match(TokenType.RBRACKET):
                    while True:
                        if self.match(TokenType.SPREAD):
                            self.advance()
                            rest_name = self.consume(TokenType.IDENTIFIER, "Expected variable name after '...'").value
                            break

                        names.append(self.consume(TokenType.IDENTIFIER, "Expected variable name").value)

                        if not self.match(TokenType.COMMA):
                            break
                        self.advance()

                self.consume(TokenType.RBRACKET, "Expected ']' after destructuring pattern")
                self.consume(TokenType.ASSIGN, "Expected '=' in assignment")
                value = self.parse_expression()
                self.optional_semicolon()
                return ArrayDestructure(names, rest_name, value)
            ...
            if self.match(TokenType.LBRACE):
                ...
                if not self.match(TokenType.RBRACE):
                    names.append(self.consume(TokenType.IDENTIFIER, "Expected property name").value)
                    while self.match(TokenType.COMMA):
                        self.advance()
                        if self.match(TokenType.RBRACE):
                            break
                ...
                self.consume(TokenType.RBRACE, "Expected '}' after destructuring pattern")
                self.consume(TokenType.ASSIGN, "Expected '=' in assignment")
                value = self.parse_expression()
                self.optional_semicolon()
                return ObjectDestructure(names, value)
